...
class Main extends PluginBase implements Listener{
	...
	public function leaveLobby(Player $sender, Level $level){
		...
		if($this->isPlaying($sender) || $this->isWaiting($sender)){ // added starting
			...
			$sender->teleport($sender->getServer()->getDefaultLevel()->getSpawn(), 0, 0);
		...
		if(isset($this->games[$lobby])){
			if($this->isRunning($this->getLevelByName($lobby))){
				...
				if($this->isStarting($this->getLevelByName($lobby))){
					$this->games[$lobby]["status"] = "starting";
					$wait = array(30 => "30",20 => "20",10 => "10",5 => "5",4 => "4",3 => "3",2 => "2",1 => "1",0 => "GO!");
					foreach($wait as $number => $text){
						$this->getServer()->getScheduler()->scheduleDelayedTask(new Wait($this, $text, $lobby), (30 - $number) * 20);
					}
	...
	public function onLevelSave(LevelSaveEvent $event){
		$this->getLevelConfig();
		if(in_array(strtolower($event->getLevel()->getName()), array_keys($this->level->getAll())))
			;
		return;
	}
	...
	/*
	 * public function cancelChangeInventory(PlayerItemHeldEvent $event){
	 * if($this->isPlaying($event->getPlayer()) && $event->getItem()->getName() !== "Iron Shovel"){
	 * $event->getPlayer()->getInventory()->clearAll();
	 * $event->getPlayer()->getInventory()->addItem(Item::IRON_SHOVEL);
	 * $event->getPlayer()->getInventory()->setHeldItemSlot(Item::IRON_SHOVEL);
	 * $event->setCancelled(true);
	 * }
	 * }
	 */
